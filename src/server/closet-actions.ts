"use server";
import { redirect } from "next/navigation"; import { randomUUID } from "node:crypto"; import { Client } from "minio"; import { withProfile } from "./profile-session"; import { runVisionAnalysis } from "./vision-core"; import { bounce } from "./action-error";
...
export async function createItem(f:FormData){
  ...
  if(!name||!category)bounce("/closet","Nome e categoria são obrigatórios.");
  const file=f.get("photo");
  const hasPhoto=file instanceof File && file.size>0;
  if(hasPhoto&&!(file as File).type.startsWith("image/"))bounce("/closet","Envie um arquivo de imagem.");
