...
async function makeHash(p:string){if(p.length<10)throw new Error("A senha deve ter pelo menos 10 caracteres.");const salt=randomBytes(16),key=await scrypt(p,salt,64) as Buffer;return `scrypt$${salt.toString("hex")}$${key.toString("hex")}`}
...
export async function register(f:FormData){const email=String(f.get("email")||"").trim().toLowerCase(),name=String(f.get("name")||"").trim(),ph=await makeHash(String(f.get("password")||""));if(!/^\S+@\S+\.\S+$/.test(email)||!name)throw new Error("Informe nome e e-mail válidos.");const c=getPool();const x=await c.connect();try{await x.query("BEGIN");const id=(await x.query("SELECT user_id FROM register_account($1,$2,$3)",[email,ph,name])).rows[0].user_id;await issue(x,id);await x.query("COMMIT");redirect("/onboarding")}catch(e){try{await x.query("ROLLBACK")}catch{}if((e as any).code==="23505")throw new Error("Este e-mail já está cadastrado.");throw e}finally{x.release()}}
export async function login(f:FormData){const email=String(f.get("email")||"").trim().toLowerCase(),p=String(f.get("password")||""),c=await getPool().connect();try{const q=await c.query("SELECT id,password_hash FROM lookup_login($1)",[email]);if(!q.rowCount||!(await checkHash(p,q.rows[0].password_hash)))redirect("/?error=credentials");await issue(c,q.rows[0].id);redirect("/onboarding")}finally{c.release()}}
