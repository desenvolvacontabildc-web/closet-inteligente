...
export async function addPartnerItem(f: FormData) {
  ...
  if (!name) bounce("/parceiras/painel", "Informe o nome da peça.");
  let objectKey = "", contentType = "";
  if (file instanceof File && file.size > 0) {
    if (!file.type.startsWith("image/")) bounce("/parceiras/painel", "Envie uma imagem.");
  ...
  let result;
  try {
    result = await withPartner(async (c, partnerId, _store, pkg) => {
    ...
    bounce("/parceiras/painel", e?.message?.replace(/^.*?ERROR:\s*/, "") || "Não foi possível publicar a peça.");
  }
