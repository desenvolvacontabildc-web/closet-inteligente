...
export async function generateCapsule(f: FormData) {
  ...
  await withProfile(async (c, userId) => {
    ...
    if (!hasPlanAtLeast(sub, "FASHION")) throw new Error("O Closet Cápsula é exclusivo dos planos Fashion e Super Star (ou do teste gratuito). Fale com a administradora para migrar de plano.");
    ...
    if (items.length === 0) throw new Error("Cadastre peças no closet antes de gerar uma cápsula.");
    ...
    let parsed: any;
    try { parsed = JSON.parse(out.output_text); } catch { throw new Error("A IA não retornou uma cápsula válida. Tente novamente."); }
    ...
    if (ids.length === 0) throw new Error("Não foi possível montar uma cápsula com as peças atuais do seu closet.");
