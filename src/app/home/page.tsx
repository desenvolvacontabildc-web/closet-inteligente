...
const PLAN_LABEL: Record<string,string> = { ESSENCIAL: "Closet Essencial", ICON: "Closet Icon" };
export default async function Home(){
 const profile=await withProfile(async(c,id)=>{
   ...
   return {...p,sub,aiRemaining};
 });
 if(!profile)redirect("/");
 if(!profile.onboarding_completed)redirect("/onboarding");
 const {display_name,experience_tokens,is_admin,sub,aiRemaining}=profile;
 ...
 return <main className="shell" style={{"--accent":experience_tokens?.accent||"#b25b76"} as React.CSSProperties}><div className="top"><span className="eyebrow">CLOSET INTELIGENTE</span><form action={logout}><button className="link">Sair</button></form></div>{trialDaysLeft!==null&&<div className="trial-banner"><p>{trialDaysLeft>0?`Faltam ${trialDaysLeft} dia${trialDaysLeft===1?"":"s"} do seu teste gratuito.`:"Seu teste gratuito termina hoje."} Fale com a administradora para assinar e manter o acesso ao seu Closet.</p></div>}{sub?.status==="ACTIVE"&&<div className="trial-banner"><p>Plano {PLAN_LABEL[sub.plan]||sub.plan}{aiRemaining!==null?` · restam ${aiRemaining} usos de IA este mês`:" · usos de IA ilimitados"}.</p></div>}<section className="welcome"><p className="eyebrow">SEU CLOSET ESTÁ PRONTO</p><h1>Olá, {display_name}.</h1><p>Preparamos uma experiência com a sua identidade. O próximo passo é adicionar suas peças.</p><div className="empty"><h2>Seu closet começa aqui</h2><p>Em breve você poderá cadastrar fotos e organizar suas peças reais.</p><Link href="/closet">Abrir meu Closet</Link><Link href="/looks">Meus Looks</Link>{is_admin&&<Link href="/admin">Administração de contas</Link>}</div></section></main>}
