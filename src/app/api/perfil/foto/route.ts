...
export async function GET(){
  const result=await withProfile(async (c,userId)=>{
    const q=await c.query("SELECT avatar_object_key,avatar_content_type FROM profiles WHERE user_id=$1",[userId]);
    if(!q.rowCount||!q.rows[0].avatar_object_key)return null;
    return {key:q.rows[0].avatar_object_key,contentType:q.rows[0].avatar_content_type||"image/jpeg"};
  });
