...
export async function GET(){
  const result=await withProfile(async (c,userId)=>{
    const q=await c.query("SELECT avatar_illustration_object_key FROM profiles WHERE user_id=$1",[userId]);
    if(!q.rowCount||!q.rows[0].avatar_illustration_object_key)return null;
    return {key:q.rows[0].avatar_illustration_object_key};
  });
  ...
  return new NextResponse(new Uint8Array(body),{headers:{"Content-Type":"image/png","Cache-Control":"private, max-age=3600"}});
}
